...
void PlaylistNode::DeleteSong(PlaylistNode* entryNode) {
	...
	if (entryNode->nextNodePtr == 0) {
		cout << "ERROR: Playlist is empty, nothing to delete... you monster..." << endl << endl;
		return;
		}
	...
	if (found == true) {
		...
		cout << "\"" << currNode->songName << "\"" << " will be deleted, are you sure? (y/n): ";
		cin >> userChoice;
		...
		if (userChoice == 'y') {
			cout << "\"" << currNode->songName << "\" was deleted" << endl << endl;
...
void PlaylistNode::PrintPlaylist(PlaylistNode* headObj) {
	...
	if (headObj->nextNodePtr == 0) {
		cout << "Playlist is empty, fill it up!!" << endl << endl;
		}
...
void PlaylistNode::ChangePositions(PlaylistNode* headNode) {
	...
	cout << endl << endl << " Enter new position for song \"" << targetNode->songName << "\": ";
	cin >> newSongPos;
	while (cin.fail()) {
		cin.clear();																// Determines whether input is valid
		cin.ignore(999, '\n');
	...
	if (newSongPos < 1) {																// Sets value to 1 or max value if input is too high
		newSongPos = 1;
		}
